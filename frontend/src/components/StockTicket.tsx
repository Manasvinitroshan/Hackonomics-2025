...
// We still want three identical rows; the middle row (idx === 1) goes in reverse.
const tickerRows: StockItem[][] = [stockData, stockData, stockData];
...
const StockTicker: FC = () => (
  <div className="ticker-container">
    {tickerRows.map((row, idx) => (
      <div
        key={idx}
        className={`ticker-row${idx === 1 ? " reverse" : ""}`}
      >
        <div className="ticker-content">
          ...
          {[...row, ...row].map((item, i) => (
            <div
              ...
              key={`${idx}-${i}`}
              style={{ color: "#FFFFFF" }} // set text to white
            >
